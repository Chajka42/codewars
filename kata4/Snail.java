...
public class Snail {
    public static int[] snail(int[][] array) {
        ...
        int[][] next = array;
        while (next != null && next.length > 1) {next = copyNext(next);
        if (next != null) {next = rotateCounterClockwise(next); list.add(Arrays.stream(next[0]).boxed().collect(Collectors.toList()));}}
    ...
    private static int[][] copyNext(int[][] array) {
        ...
        for (int i = 1; i < array.length; i++) {
            for (int j = 0; j < array[0].length; j++) {newArray[i - 1][j] = array[i][j];}}
        return newArray;}
